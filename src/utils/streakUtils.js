export const getChicagoDate = () => {
  ...
  const chicagoDate = new Date(new Date().getTime() + chicagoTimeOffset * 60 * 1000);
...
export const updateStreakDays = (user, countChange) => {
  ...
  const completedDays = user.streak?.completedDays || {}; // {date: count, ...}
  const streakCount = user.streak?.count || 0; // Initial streak count if not set yet
